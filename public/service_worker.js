...
self.addEventListener("fetch", (event) => {
  ...
  if (event.request.url === nonExistentAPI && event.request.method === "GET") {
    ...
    event.respondWith(
      new Response(
        JSON.stringify({
          message:
            "This is dummy API response you get as you are calling a non exist API ",
        }),
        {
  ...
  if (
    ...
    event.request.url.includes(catUrlToCache)
  ) {
  ...
  event.respondWith(
    fetch(event.request)
      .then((res) => res)
      .catch((err) => console.log("Error Other Cases last section: ", err))
  );
});
